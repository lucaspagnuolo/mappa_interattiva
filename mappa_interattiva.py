...
def genera_mappa_concettuale(testo: str, central_node: str, index_terms: list[str]=None) -> dict:
    ...
    ris = []; status = st.empty(); progress = st.progress(0)
    totale = len(blocchi)
    for idx,b in enumerate(blocchi,1):
        pct = int(((idx-1)/totale)*100); status.info(f"Generazione mappa... {pct}%"); progress.progress(pct)
        ...
        if txt.startswith("```"): lines = txt.splitlines(); txt = "\n".join(lines[1:-1])
        start,end = txt.find('{'), txt.rfind('}')+1
        raw = txt[start:end] if start!=-1 and end!=-1 else ''
        try: ris.append(json.loads(raw))
        except json.JSONDecodeError: st.warning(f"Parsing fallito per blocco {idx}")
    status.success("Mappa concettuale generata"); progress.empty()
    ...
    for m in ris:
        for n in m.get('nodes',[]):
            ...
            if isinstance(nid,str):
                nid_str=nid.strip()
                if nid_str and not re.match(r'^(?:\d+|n\d+)$',nid_str,flags=re.IGNORECASE): raw_nodes.add(nid_str)
    ...
    idxs=index_terms or[]; filt=filtra_paragrafi_sottoparagrafi(idxs); BOOST=5
...
def crea_grafo_interattivo(mappa: dict, central_node: str, soglia: int) -> str:
    ...
    html_file=f"temp_graph_{int(time.time())}.html"; net.save_graph(html_file); st.success("Grafo generato")
    return html_file
...
if mode == "Mappa Gerarchica":
    ...
    if st.button("Genera JSON Completo") and doc:
        ...
        if os.path.exists(gif_path): b64=base64.b64encode(open(gif_path,'rb').read()).decode(); gif_ph.markdown(f"<img src='data:image/gif;base64,{b64}' width=200/>",unsafe_allow_html=True)
        testo=estrai_testo_da_pdf(doc); index_terms=estrai_indice(testo)
        st.session_state['mappa']=genera_mappa_concettuale(testo,central_node,index_terms)
        st.session_state['testo']=testo; st.session_state['central_node']=central_node; st.session_state['index_terms']=index_terms
        gif_ph.empty(); st.subheader("JSON Completo"); st.json(st.session_state['mappa'])
