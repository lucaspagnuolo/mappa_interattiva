...
def call_with_retries(prompt_args, max_retries=5):
    for attempt in range(1, max_retries + 1):
        try:
            ...
            st.error(f"Errore API (SDKError): {e}")
            if e.status_code == 429 and attempt < max_retries:
                time.sleep(2 ** attempt)
                continue
            raise
        except Exception as e:
            st.error(f"Errore API generico: {e}")
...
def genera_mappa_concettuale(testo: str, central_node: str) -> dict:
    ...
    for idx, b in enumerate(blocchi, 1):
        prompt = textwrap.dedent(f"""
            ...
            Ogni nodo deve essere una stringa. Ogni edge deve avere 'from', 'to' e 'relation'.
            Obiettivo: individuare i concetti e oggetti collegati a '{central_node}'.
...
def crea_grafo_interattivo(mappa: dict, testo: str, central_node: str, soglia: int = 1) -> str:
    ...
    net.force_atlas_2based(gravity=-100, central_gravity=0.005, spring_length=800, spring_strength=0.002, damping=0.6)
